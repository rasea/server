...
public final class ExceptionFactory {
	...
	public static WebServiceException createNotFound(final String property, final String value) {
		...
		return new WebServiceException(String.format("%s not found %s", property, value), fault);
	}
	
	public static WebServiceException createPermissionDenied(final String authenticatedUser,
	...
	public static WebServiceException createPermissionDenied(final String username,
			final String application, final String resource, final String operation) {
		
		final ServiceFault fault = new ServiceFault();
		fault.setErrorCode(ErrorCode.PERMISSION_DENIED);
		fault.setSource(String.format("%s,%s,%s,%s", username, application, resource, operation));
		
		return new WebServiceException(String.format("permission denied [%s,%s,%s] to %s",
				application, resource, operation, username), fault);
	}
