...
public abstract class AbstractViewHelper implements Serializable {
	...
	/**
	 * Verifica a permissão do usuário para executar ações de uma tela. Se quiser saber mais é
	 * respeito, leia sobre o Sem Security.
	 * 
	 * @param action
	 *            Ação que será verificada para responder a pergunta: o usuário pode acessar a tela
	 *            executando a ação?
	 */
	...
	/**
	 * Quando você não estiver satisfeito com a implementação padrão do tratamento de exceção,
	 * sobrescreva este método. O método trataExcecao() aplica um tratamento padrão que é o de
	 * exibir uma mensagem na tela relacionada com a exceção lançada.
	 * 
	 * @see #performExceptionHandling(Exception)
	 */
	public void handleException(final AbstractApplicationException cause)
	...
	/**
	 * <p>
	 * Faz o tratamento de exceções de RuntimeException, tratando-as devidamente. As exceções
	 * BusinessException serão sempre convertidas em mensagens genéricas na tela, e devem ser
	 * lançadas apenas pelas classes da camada de negócio.
	 * </p>
	 * <p>
	 * Caso a exceção não seja tratada, ou caso ela não seja uma RuntimeException, o tratamento será
	 * dado pelo Seam. O mapeamento de exceções do Seam fica no arquivo de configuração
	 * <i>pages.xml</i>.
	 * </p>
	 * 
	 * @param cause
	...
	protected void performExceptionHandling(final Exception cause) throws UncaughtException {
		...
		if (cause instanceof DuplicatedException) {
			this.statusMessages.addToControlFromResourceBundle(((DuplicatedException) cause)
					.getProperty(), cause.getMessage());
	...
	private void takeCareOf(final AbstractBusinessException exception) {
		this.getStatusMessages().addFromResourceBundle(
				Severity.valueOf(exception.getSeverity().name()), exception.getMessage(),
				(Object[]) exception.getParams());
	}

	private void takeCareOf(final AbstractInvalidPropertyException exception) {
		this.getStatusMessages().addToControlFromResourceBundle(exception.getProperty(),
				Severity.valueOf(exception.getSeverity().name()), exception.getMessage(),
				(Object[]) exception.getParams());
	}
	...
	private void takeCareOf(final ViewException exception) {
		if (exception.getFields() != null) {
			for (final Object field : exception.getFields()) {
				this.getStatusMessages().addToControlFromResourceBundle(field.toString(),
						Severity.valueOf(exception.getSeverity().name()), exception.getMessage(),
						(Object[]) exception.getParams());
			}
		} else {
